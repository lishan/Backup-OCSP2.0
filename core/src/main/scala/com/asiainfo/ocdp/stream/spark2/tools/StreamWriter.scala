package com.asiainfo.ocdp.stream.spark2.tools
...
import com.asiainfo.ocdp.stream.tools.{Json4sUtils, KafkaSendTool}
...
class StreamKafkaWriter(diConf: DataInterfaceConf) extends StreamWriter with Logging{
  ...
  def push(df: DataFrame, conf: EventConf, uniqKeys: String) = setMessage(df.toJSON.rdd, conf, uniqKeys).count
