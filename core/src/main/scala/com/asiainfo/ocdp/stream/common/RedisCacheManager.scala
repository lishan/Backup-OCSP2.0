...
abstract class RedisCacheManager(sysProps: SystemProps) extends CacheManager {
  val logger = LoggerFactory.getLogger(this.getClass)
  val systemProps = sysProps
  ...
  val miniBatch = systemProps.getInt("cacheQryTaskSizeLimit")
  ...
  override def getMultiCacheByKeys(keys: List[String],
    ...
    try {
      ...
      case e2: Exception =>{logger.error("RedisCacheManager.getMultiCacheByKeys－Exception异常！", e2.getStackTrace())
        ...
        val sb = new StringBuffer()
        val stackArray = e2.getStackTrace()
        stackArray.foreach( stack => sb.append(stack.toString + "\n"))
        println("exception:" + sb.toString)
    }
